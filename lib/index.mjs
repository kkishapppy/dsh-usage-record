...
function apply(ctx, config = {}) {
	...
	*  O(新增) 而非 O(全量)。回扫上限 5000 条防异常乱序（无 seq 事件混排）。 */
	function extractTailQuestions(events, fromSeq) {
		...
		let scanned = 0;
		while (i >= 0 && scanned < 5e3) {
			const e = events[i];
			if ((typeof e?.seq === "number" ? e.seq : 0) <= fromSeq) break;
			i--;
			scanned++;
		}
	...
	*  从磁盘读日志（sessionPersistence.loadStored），保证历史提问不丢。 */
	...
	async function questionsFor(sessionId) {
		...
		if (indexed !== null && indexed.questions.length > 0) {
			...
			let newOnes = [];
			if (indexed.lastSeq !== null) try {
				const live = typeof ctx.sessions?.get === "function" ? ctx.sessions.get(sessionId) : void 0;
				if (live && Array.isArray(live.log) && live.log.length > 0) newOnes = extractTailQuestions(live.log, indexed.lastSeq);
			} catch {}
			if (newOnes.length > 0) {
